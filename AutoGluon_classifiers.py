...
# basic: 0-14; 3D_A: 14-526; 3D_C: 526-1038; 3D_V: 1038-1550; RA_A: 1550-3331; RA_C: 3331-5112; RA_V: 5112-6893
data_s = data.iloc[:, np.r_[0:14, 14:526, 1038:1550]]
data_s = data_s.dropna()

data_enroll = data_s[data_s.diameter <= 210]
data_patient = data_enroll[['id', 'diameter', 'patho_msubtype', 'index']]
data_v = data_enroll.drop(columns=['id', 'mask', 'gender', 'age',  'laterality', 'diameter', 'a_phase', 'c_phase', 'v_phase', 'image_class_pos', 'image_class_une', 'patho_msubtype', 'patho_auditTime', 'index'])
## end ##
...
data_vali_x = data_vali.drop(columns=['id', 'diameter', 'patho_msubtype', 'index'])
...
data_test_x = data_test.drop(columns=['id', 'diameter', 'patho_msubtype', 'index'])
...
# fea_imp = predictor.feature_importance(data_vali)
# pd.DataFrame(fea_imp).to_csv("D:\PyProject\ReTumor\AutoGluon\Results_ST\RAD\FeaSel_importance_RA_cp30_M01.csv")
...
# pd.DataFrame(y_vali_prob).to_csv("D:\PyProject\ReTumor\AutoGluon\Results_ST\ACC\data_3D_nb_vali_cp30_M_prob.csv")
# pd.DataFrame(y_vali_cate).to_csv("D:\PyProject\ReTumor\AutoGluon\Results_ST\ACC\data_3D_nb_vali_cp30_M_cate.csv")
#
# pd.DataFrame(y_test_prob).to_csv("D:\PyProject\ReTumor\AutoGluon\Results_ST\ACC\data_3D_nb_test_cp30_M_prob.csv")
# pd.DataFrame(y_test_cate).to_csv("D:\PyProject\ReTumor\AutoGluon\Results_ST\ACC\data_3D_nb_test_cp30_M_cate.csv")